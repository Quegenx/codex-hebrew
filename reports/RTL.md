...
The visual matrix remains incomplete. Dark mode, signed-in home and conversations, settings, plugin/skill views, menus, dialogs, tooltips, focus states, loading and empty states have no direct capture. There is also no Windows runtime or visual evidence. `bun run visual:check` and `bun run verify` therefore exit nonzero and expose these missing categories in `coverage.json`.
